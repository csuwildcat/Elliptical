    ...
    if (routeMatched) {
      ...
      if (Router.initialized){
        if ((newState.href !== oldState.href) && (event ? event.type !== 'popstate' : event === false)) {
          history.pushState(newState, 'Elliptical' + (newState.title ? ' - ' + newState.title : ''), newState.search);
        }
      }
      else {
        history.replaceState(newState, 'Elliptical' + (newState.title ? ' - ' + newState.title : ''), newState.search);
      }
